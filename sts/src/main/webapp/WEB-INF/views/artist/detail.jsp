...
<body>
	...
	<div class="artist_detail">
		<div class="left">
			...
			<div class="btnbtn">
				<a
					href="<c:url value='/artist/update?artist_idx=${detailArtist.artist_idx}'/>">수정</a>
				<a
					href="<c:url value='/artist/artistDelete?artist_idx=${detailArtist.artist_idx}'/>">삭제</a>
			</div>
		</div>
		<div class="middle">
				...
				<c:choose>
					<c:when test="${hashMap.member_idx ne null}">
						<a href='javascript: like_func();'><img
							src="${path}/resources/img/empty_heart.png" id='like_img'></a>
					</c:when>
					<c:otherwise>
						<a href='javascript: login_need();'><img
							src="${path}/resources/img/heart.png"></a>
					</c:otherwise>
				</c:choose>
				<a href="" onclick="clip(); return false;"><img
					src="${path}/resources/img/share.png" alt="welp"></a> <img
					src="${path}/resources/img/home.png" alt="welp">
			</div>
		</div>
	</div>
	<div id="reply">
				...
				<c:forEach items="${reply}" var="reply">
					<div id="replyList">
						...
						<div id="replyAdditional">
							<a id="replyEdit">수정</a><span>&nbsp;|&nbsp;</span><a
								id="replyDelete"> 삭제</a>
		...
		<div class="replyWrite">
			...
			<input type="hidden" id="member_name" name="member_name"
				value="${hashMap.member_loginId}">
		</div>
		<textarea
			placeholder="
	   댓글 내용을 입력해주세요.
	   (무분별한 악의적 댓글 및 허위 사실 유포성 댓글을 통보없이 삭제될 수 있습니다.)"
			id="reply_content" name="reply_content" rows="5" cols="50"></textarea>
	...
	<div class="edit">
		<sec:authorize access="isAuthenticated() and hasRole('ROLE_ADMIN')">
			<a
				href="<c:url value='/artist/registrationWork?artist_idx=${detailArtist.artist_idx}'/>">작품등록</a>
		</sec:authorize>
	</div>
	<div class="list">
