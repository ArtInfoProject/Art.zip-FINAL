...
<body>
	...
	<sec:authorize access="isAuthenticated() and hasRole('ROLE_ADMIN')">
		<div class="edit">
			<a href="/contest/update?contest_idx=${detail.contest_idx}">수정</a> <a
				href="/contest/delete?contest_idx=${detail.contest_idx}">삭제</a>
		</div>
	</sec:authorize>
	<div class="ex_detail_contents">
		...
		<p class="location">
			<br>${detail.contest_region}<br> ${detail.contest_location}
		</p>
	...
	<div class="article">
		<table>
			<c:forEach var="data" items="${list}">
				<c:if
					test="${fn:length(list) == 1 and data.contest_idx < detail.contest_idx}">
					<tr>
						<td>다음글</td>
						<td>마지막 글입니다.</td>
					</tr>
				</c:if>
				<c:if test="${data.contest_idx < detail.contest_idx}">
					<tr>
						<td>이전글</td>
						<td><a href="/contest/detail?contest_idx=${data.contest_idx}">${data.contest_title}</a></td>
					</tr>
				</c:if>
				<c:if test="${data.contest_idx > detail.contest_idx}">
					<tr>
				...
				<c:if
					test="${fn:length(list) == 1 && data.contest_idx > detail.contest_idx}">
					<tr>
						<td>이전글</td>
						<td>이전글이 없습니다.</td>
					</tr>
